...
uint8_t get_pseudo_random_byte(void);
...
// Global interrupt enable
    ...
    // info
    uint8_t pt[1600] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
    ...
    for (;;) {
        ...
        if (!strcmp(command, "pt")) {
            ...
            do_encryption_decryption(pt, ad, plaintext_byte_len,
                                     associated_data_byte_len, 1);
        }

        else if (!strcmp(command, "kat")) {
            for (int j = 0; j < 33; j++) {
                for (int i = 0; i < 33; i++) {
                    do_encryption_decryption(pt, ad, j, i, 1);
                }
            }
            ...
            int benchmarks[6] = {1, 8, 16, 32, 64, 1536};

            for(int i = 0; i < 1600; i++){
                pt[i] = get_pseudo_random_byte();
            }

            for (int i = 0; i < 6; i++) {
                neorv32_uart0_printf( "============================\n"
                                            "Performing benchmark for %d bytes\n"
                                            "============================\n"
                    ,
                                     benchmarks[i]);
